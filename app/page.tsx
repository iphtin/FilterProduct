...
const SUBCATEGORY = [
   ...
   {name: "Sweatshirt", selected: false, href: "#"},
   {name: "Acessories", selected: false, href: "#"}
]
...
export default function Home() {
  const [filter, setFilter] = useState<ProductState>({
    color: ["beige", "blue", "purple", "white", "green"],
  ...
  const applyArrayFilter = ({
    ...
    const isFilterApplie = filter[category].includes(value as never)
    ...
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
       <div className="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-24">
         <h1 className="text-4xl font-bold tracking-tight text-gray-900">
          High Quality  cotton selecttion
         </h1>
            ...
            <DropdownMenuContent align="end">
              {SORT_OPTIONS.map((option) => (
                <button key={option.name} onClick={() => {
                  ...
                  _debouncedSubmit()
                }} className={cn('text-left w-full block text-sm px-4 py-4',
           ...
           <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
              ...
              <Accordion type="multiple" className="animate-none">
                 ...
                 <AccordionItem value="color">
                    ...
                    <AccordionContent className="pt-6 pb-4 animate-none">
                    ...
                    <AccordionContent className="pt-6 pb-4 animate-none">
                      <ul className="space-y-4">
                        ...
                        <li className="flex justify-center flex-col gap-2">
                          <div>
                          <input type="radio" id={`price-${PRICE_FILTER.options.length}`} 
                          onChange={() => {
                             setFilter((prev) => ({
                               ...prev,
                               price: {
                          ...
                          className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                           <label htmlFor={`price-${PRICE_FILTER.options.length}`} className="ml-3 text-sm text-gray-600">
                            Custom
                            </label>
                          </div>
              ...
              <ul className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 
